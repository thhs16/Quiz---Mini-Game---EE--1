...
namespace MaterialUI
{
	public class ShadowConfig : MonoBehaviour, IPointerEnterHandler, IPointerDownHandler, IPointerUpHandler, IPointerExitHandler
	{
		...
		{
			// Safety Check: ensure the array isn't empty before trying to use it
			if (isEnabled && shadows != null)
			{
				foreach (ShadowAnim shadow in shadows)
				{
					if (shadow != null) shadow.SetShadow(false);
				}
				
				// Calculate the index first
				int index = shadowOn - 1;

				// Check if the index is valid (inside the list size)
				if (index >= 0 && index < shadows.Length)
				{
					if (shadows[index] != null)
					{
						shadows[index].SetShadow(true);
					}
				}
			}
		}
